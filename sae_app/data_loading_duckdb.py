"""Reading and validating the CSV data files -- DuckDB-backed implementation.
...
Row-level operations -- required-code normalization, duplicate-key conflict
detection, table joins, coordinate coalescing/quality checks, and the
calibration validation queries -- run as SQL through sae_app/db.py.

sae_app/data_loading.py dispatches to this module or to
sae_app/data_loading_pandas.py based on sae_app.constants.USE_DUCKDB. Do not
import this module directly -- import sae_app.data_loading instead, so the
mode switch keeps working. Importing this module requires the duckdb package
to be installed; the pandas module does not.
"""
...
def load_program_names(file_bytes: bytes) -> pd.DataFrame:
    ...
    with db.connect() as con:
        ...
        db.register_df(con, "_names_src", working)
...
def validate_cumulative_share_columns(
    ...
    *,
    ...
    with db.connect() as con:
        db.register_df(con, "_share_src", calib)
...
def validate_core_numeric_columns(
    ...
    *,
    ...
    with db.connect() as con:
        db.register_df(con, "_numeric_src", calib)
